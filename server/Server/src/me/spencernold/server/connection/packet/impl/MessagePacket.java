				...
				try {
					...
					for (String s : conversation.getUsers()) {
						...
						if (c == null)
							continue;
						if (type == 1) {
							c.send(new MessagePacket(convo, from, message.getIndex(), (byte) 2, new byte[0], new byte[0]));
							c.send(new MessagePacket(convo, from, message.getIndex(), (byte) 3, message.getData(), message.getSignature()));
							continue;
						}
